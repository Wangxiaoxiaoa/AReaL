...
from areal.infra.rpc.serialization import deserialize_values, serialize_value
...
class TestRTensorIntegration:
    ...
    def test_batch_shard_retrieval(self, rpc_server):
        ...
        localized = deserialize_values(serialized_batch)
...
class TestHttpRTensorBackendBatching:
    ...
    def test_fetch_chunks_large_requests(self, monkeypatch):
        ...
        shards = [TensorShardInfo(shard_id=f"s{i}", node_addr="node-a") for i in range(5)]
        requested_chunks = []
        ...
        async def fake_fetch_shard_group(self, session, node_addr, grouped):
            requested_chunks.append((node_addr, [shard.shard_id for _, shard in grouped]))
            return [torch.tensor([int(shard.shard_id[1:])]) for _, shard in grouped]

        monkeypatch.setattr(
            "areal.infra.rpc.rtensor.aiohttp.ClientSession",
            lambda: _FakeSession(),
        )
        monkeypatch.setattr(
            backend,
    ...
    def test_fetch_shard_group_falls_back_on_legacy_404(self, monkeypatch):
        """404 on /data/batch falls back to one-request-per-shard fetches."""
        ...
        async def fake_fetch_tensor(self, session, shard_id, node_addr):
            fallback_calls.append((shard_id, node_addr))
            return torch.tensor([int(shard_id[1:])])

        monkeypatch.setattr(
            backend,
            "_fetch_tensor",
            fake_fetch_tensor.__get__(backend, HttpRTensorBackend),
        )

        tensors = asyncio.run(backend._fetch_shard_group(_FakeSession(), "node-a", grouped))

        assert fallback_calls == [("s0", "node-a"), ("s1", "node-a")]
        assert [int(tensor.item()) for tensor in tensors] == [0, 1]
